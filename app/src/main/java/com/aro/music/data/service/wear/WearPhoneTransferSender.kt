...
import com.aro.music.shared.WearCapabilities
...
class WearPhoneTransferSender @Inject constructor(
    ...
    suspend fun isAroWatchAvailable(): Boolean {
        return runCatching {
            ...
            Timber.tag(TAG).w(error, "Failed checking Aro Wear availability")
            false
        }
    }
    ...
    suspend fun requestSongTransfer(songId: String, songTitle: String = ""): Result<Int> {
        ...
        return runCatching {
            ...
            if (nodes.isEmpty()) {
                error("No reachable watch with Aro")
            }
