...
class ImageCacheManager @Inject constructor(
    ...
    fun invalidateCoverArtCaches(vararg uriStrings: String?) {
        ...
        uriStrings.mapNotNull { it?.takeIf(String::isNotBlank) }.forEach { baseUri ->
            if (com.aro.music.utils.LocalArtworkUri.isLocalArtworkUri(baseUri)) {
                com.aro.music.utils.LocalArtworkUri.parseSongId(baseUri)?.let { songId ->
                    com.aro.music.utils.AlbumArtUtils.clearCacheForSong(context, songId)
                }
            }
