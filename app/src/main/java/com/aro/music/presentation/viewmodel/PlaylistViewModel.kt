...
import com.aro.music.data.DailyMixManager
import com.aro.music.data.model.Playlist
import com.aro.music.data.model.SmartPlaylistRule
import com.aro.music.data.model.Song
import com.aro.music.data.model.SortOption
import com.aro.music.data.playlist.M3uManager
...
import com.aro.music.data.preferences.TelegramTopicDisplayMode
import com.aro.music.data.ai.AiPlaylistGenerator
...
class PlaylistViewModel @Inject constructor(
    ...
    private fun findFolder(
        targetPath: String,
        folders: List<com.aro.music.data.model.MusicFolder>
    ): com.aro.music.data.model.MusicFolder? {
        val queue: ArrayDeque<com.aro.music.data.model.MusicFolder> = ArrayDeque(folders)
    ...
    private fun com.aro.music.data.model.MusicFolder.collectAllSongs(): List<Song> {
    ...
    private fun sortPlaylistsList(
        playlists: List<com.aro.music.data.model.Playlist>,
        sortOption: SortOption
    ): List<com.aro.music.data.model.Playlist> {
        return when (sortOption) {
            SortOption.PlaylistNameAZ -> playlists.sortedWith(
                compareBy<com.aro.music.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistNameZA -> playlists.sortedWith(
                compareByDescending<com.aro.music.data.model.Playlist> { it.name.lowercase() }
                    .thenByDescending { it.lastModified }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreated -> playlists.sortedWith(
                compareByDescending<com.aro.music.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
            SortOption.PlaylistDateCreatedAsc -> playlists.sortedWith(
                compareBy<com.aro.music.data.model.Playlist> { it.lastModified }
                    .thenBy { it.name.lowercase() }
                    .thenBy { it.id }
            )
    ...
    fun exportPlaylistsAsM3u(playlistIds: List<String>) {
        ...
        viewModelScope.launch {
            try {
                ...
                val exportDir = File(musicDir, "Aro Exports")
                if (!exportDir.exists()) {
                    exportDir.mkdirs()
                }
